...
mod tests {
    ...
    #[test]
    fn test_is_retryable_status_all_5xx() {
    ...
    #[test]
    fn test_is_retryable_status_200_ok_not_retryable() {
        assert!(!is_retryable_status(200));
    }

    #[test]
    fn test_is_retryable_status_400_bad_request_not_retryable() {
        assert!(!is_retryable_status(400));
    }

    #[test]
    fn test_is_retryable_status_401_unauthorized_not_retryable() {
        assert!(!is_retryable_status(401));
    }

    #[test]
    fn test_is_retryable_status_403_forbidden_not_retryable() {
        assert!(!is_retryable_status(403));
    }

    #[test]
    fn test_is_retryable_status_404_not_found_not_retryable() {
        assert!(!is_retryable_status(404));
    }

    #[test]
    fn test_is_retryable_status_422_unprocessable_not_retryable() {
    ...
    #[test]
    fn test_extract_invalid_nonce_valid_structure() {
        ...
        if let Some(RpcError::InvalidNonce { tx_nonce, ak_nonce }) = result {
    ...
    #[test]
    fn test_extract_invalid_nonce_missing_tx_execution_error() {
        let data = serde_json::json!({
            "SomeOtherError": {}
        });
    ...
    #[test]
    fn test_extract_invalid_nonce_missing_invalid_tx_error() {
        let data = serde_json::json!({
            "TxExecutionError": {
                "ActionError": {}
            }
    ...
    #[test]
    fn test_extract_invalid_nonce_missing_invalid_nonce() {
        let data = serde_json::json!({
            "TxExecutionError": {
                "InvalidTxError": {
    ...
    #[test]
    fn test_extract_invalid_nonce_missing_ak_nonce() {
        let data = serde_json::json!({
            "TxExecutionError": {
                "InvalidTxError": {
                    "InvalidNonce": {
    ...
    #[test]
    fn test_extract_invalid_nonce_missing_tx_nonce() {
        let data = serde_json::json!({
            "TxExecutionError": {
                "InvalidTxError": {
                    "InvalidNonce": {
    ...
    #[test]
    fn test_extract_invalid_nonce_non_numeric_values() {
        let data = serde_json::json!({
            "TxExecutionError": {
                "InvalidTxError": {
                    "InvalidNonce": {
                        "tx_nonce": "not a number",
                        "ak_nonce": 10
                    }
                }
            }
    ...
    #[test]
    fn test_extract_invalid_nonce_empty_data() {
        let data = serde_json::json!({});
        assert!(extract_invalid_nonce(&data).is_none());
    }

    #[test]
    fn test_extract_invalid_nonce_null_data() {
    ...
    #[test]
    fn test_rpc_client_new_with_string() {
        let url = "https://rpc.mainnet.near.org".to_string();
    ...
    fn test_rpc_client_with_retry_config() {
        ...
        let client = RpcClient::with_retry_config("https://rpc.testnet.near.org", retry_config);
        assert_eq!(client.url(), "https://rpc.testnet.near.org");
    }
    ...
    fn test_rpc_client_debug() {
        ...
        assert!(debug_str.contains("rpc.testnet.near.org"));
    ...
    fn test_mainnet_config() {
        assert!(MAINNET.rpc_url.contains("fastnear.com"));
        assert_eq!(MAINNET.network_id, "mainnet");
    }

    #[test]
    fn test_testnet_config() {
        assert!(TESTNET.rpc_url.contains("fastnear.com") || TESTNET.rpc_url.contains("test"));
        assert_eq!(TESTNET.network_id, "testnet");
    }

    // ========================================================================
    // BlockReference Tests
    // ========================================================================

    #[test]
    fn test_block_reference_final_rpc_params() {
        let block_ref = BlockReference::final_();
        let params = block_ref.to_rpc_params();
        assert_eq!(
            params.get("finality").and_then(|v| v.as_str()),
            Some("final")
        );
    }

    #[test]
    fn test_block_reference_optimistic_rpc_params() {
        let block_ref = BlockReference::optimistic();
        let params = block_ref.to_rpc_params();
        assert_eq!(
            params.get("finality").and_then(|v| v.as_str()),
            Some("optimistic")
    ...
    #[test]
    fn test_block_reference_at_height_rpc_params() {
        let block_ref = BlockReference::at_height(12345);
        let params = block_ref.to_rpc_params();
        assert_eq!(params.get("block_id").and_then(|v| v.as_u64()), Some(12345));
    }

    #[test]
    fn test_block_reference_at_hash_rpc_params() {
        let hash: CryptoHash = "4reLvkAWfqk5fsqio1KLudk46cqRz9erQdaHkWZKMJDZ"
            .parse()
            .unwrap();
        let block_ref = BlockReference::at_hash(hash);
        let params = block_ref.to_rpc_params();
        assert!(params.get("block_id").is_some());
    }
    ...
    fn test_parse_rpc_error_unknown_account() {
        ...
        // Simulate NEAR RPC error structure
        ...
        match result {
            RpcError::AccountNotFound(account_id) => {
                assert_eq!(account_id.as_str(), "nonexistent.near");
            }
            _ => panic!("Expected AccountNotFound, got {:?}", result),
        }
    }
    ...
    fn test_parse_rpc_error_invalid_account() {
        ...
        let error = JsonRpcError {
            ...
            cause: Some(ErrorCause {
                ...
                info: Some(serde_json::json!({
                    "requested_account_id": "bad..account"
                })),
            }),
            name: None,
        };
        ...
        match result {
            RpcError::InvalidAccount(account) => {
                assert_eq!(account, "bad..account");
            }
            _ => panic!("Expected InvalidAccount, got {:?}", result),
        }
    }
    ...
    fn test_parse_rpc_error_unknown_block() {
        ...
        let error = JsonRpcError {
            code: -32000,
            message: "Server error".to_string(),
            data: Some(serde_json::json!("block height 999999")),
            cause: Some(ErrorCause {
        ...
        match result {
            RpcError::UnknownBlock(block_ref) => {
                assert!(block_ref.contains("999999"));
            }
            _ => panic!("Expected UnknownBlock, got {:?}", result),
        }
    }
    ...
    fn test_parse_rpc_error_unknown_chunk() {
        ...
        match result {
            RpcError::UnknownChunk(chunk_ref) => {
                assert!(chunk_ref.contains("ChunkHash123"));
            }
            _ => panic!("Expected UnknownChunk, got {:?}", result),
        }
    }
    ...
    fn test_parse_rpc_error_unknown_epoch() {
        ...
        let error = JsonRpcError {
            code: -32000,
            message: "Server error".to_string(),
            data: Some(serde_json::json!("epoch 100")),
            cause: Some(ErrorCause {
        ...
        match result {
            RpcError::UnknownEpoch(epoch_ref) => {
                assert!(epoch_ref.contains("epoch 100"));
            }
            _ => panic!("Expected UnknownEpoch, got {:?}", result),
        }
    }
    ...
    fn test_parse_rpc_error_unknown_receipt() {
        ...
        match result {
            RpcError::UnknownReceipt(receipt_id) => {
                assert_eq!(receipt_id, "receipt123");
            }
            _ => panic!("Expected UnknownReceipt, got {:?}", result),
        }
    }
    ...
    fn test_parse_rpc_error_no_contract_code() {
        ...
        match result {
            RpcError::ContractNotDeployed(account_id) => {
                assert_eq!(account_id.as_str(), "user.near");
            }
            _ => panic!("Expected ContractNotDeployed, got {:?}", result),
        }
    }
    ...
    fn test_parse_rpc_error_too_large_contract_state() {
        ...
        match result {
            RpcError::ContractStateTooLarge(account_id) => {
                assert_eq!(account_id.as_str(), "big-contract.near");
            }
            _ => panic!("Expected ContractStateTooLarge, got {:?}", result),
        }
    }

    #[test]
    fn test_parse_rpc_error_contract_execution_error() {
        let client = RpcClient::new("http://localhost");
        ...
        match result {
            RpcError::ContractExecution {
                ...
                assert_eq!(method_name, Some("do_something".to_string()));
                assert!(message.contains("Execution failed"));
            }
            _ => panic!("Expected ContractExecution, got {:?}", result),
        }
    }
    ...
    fn test_parse_rpc_error_unavailable_shard() {
        ...
        let error = JsonRpcError {
            code: -32000,
            message: "Shard 0 is unavailable".to_string(),
            data: None,
            cause: Some(ErrorCause {
        ...
        match result {
            RpcError::ShardUnavailable(msg) => {
                assert!(msg.contains("unavailable"));
            }
            _ => panic!("Expected ShardUnavailable, got {:?}", result),
        }
    }

    #[test]
    fn test_parse_rpc_error_no_synced_blocks() {
        let client = RpcClient::new("http://localhost");
        ...
        match result {
            RpcError::NodeNotSynced(msg) => {
                assert!(msg.contains("syncing"));
            }
            _ => panic!("Expected NodeNotSynced, got {:?}", result),
        }
    }

    #[test]
    fn test_parse_rpc_error_not_synced_yet() {
        let client = RpcClient::new("http://localhost");
    ...
    fn test_parse_rpc_error_invalid_shard_id() {
        ...
        let error = JsonRpcError {
            code: -32000,
            message: "Invalid shard".to_string(),
            data: None,
            cause: Some(ErrorCause {
        ...
        match result {
            RpcError::InvalidShardId(shard_id) => {
                assert!(shard_id.contains("999"));
            }
            _ => panic!("Expected InvalidShardId, got {:?}", result),
        }
    }

    #[test]
    fn test_parse_rpc_error_invalid_transaction_with_nonce() {
        let client = RpcClient::new("http://localhost");
        ...
        match result {
            ...
            _ => panic!("Expected InvalidNonce, got {:?}", result),
        }
    }

    #[test]
    fn test_parse_rpc_error_timeout_error() {
        let client = RpcClient::new("http://localhost");
        ...
        match result {
            RpcError::RequestTimeout {
                ...
                assert_eq!(transaction_hash, Some("tx123".to_string()));
            }
    ...
    fn test_parse_rpc_error_internal_error() {
        ...
        match result {
            ...
            _ => panic!("Expected InternalError, got {:?}", result),
        }
    }

    #[test]
    fn test_parse_rpc_error_account_not_found_from_data_string() {
        let client = RpcClient::new("http://localhost");

        // Fallback parsing from data string
        let error = JsonRpcError {
            code: -32000,
            message: "Server error".to_string(),
            data: Some(serde_json::json!(
        ...
        match result {
            RpcError::AccountNotFound(account_id) => {
                assert_eq!(account_id.as_str(), "missing.near");
            }
            _ => panic!("Expected AccountNotFound, got {:?}", result),
        }
    }
    ...
    #[test]
    fn test_parse_rpc_error_generic_rpc_error() {
        let client = RpcClient::new("http://localhost");

        let error = JsonRpcError {
            code: -32600,
            message: "Unknown error".to_string(),
            data: Some(serde_json::json!({"details": "some info"})),
            cause: None,
            name: None,
        };
        ...
        match result {
            ...
            _ => panic!("Expected Rpc, got {:?}", result),
        }
    }

    #[test]
    fn test_parse_rpc_error_unknown_cause_name() {
        let client = RpcClient::new("http://localhost");
        ...
        let error = JsonRpcError {
            ...
            data: None,
            cause: Some(ErrorCause {
                name: "SOME_NEW_ERROR_TYPE".to_string(),
                info: None,
            }),
            name: None,
        };
        ...
        // Should fall through to generic Rpc error
        match result {
            RpcError::Rpc { code, message, .. } => {
                assert_eq!(code, -32000);
                assert_eq!(message, "Some error");
            }
            _ => panic!("Expected Rpc fallback, got {:?}", result),
        }
    }
}
